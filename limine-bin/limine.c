...
static struct gpt2mbr_type_conv gpt2mbr_type_conv_table[] = {
	{ 0x11d2f81fc12a7328, 0x3bc93ec9a0004bba, 0xef },

	{ 0x4433b9e5ebd0a0a2, 0xc79926b7b668c087, 0x07 },

	{ 0x11aa000048465300, 0xacec4365300011aa, 0xaf },
...
static bool _device_write(const void *_buffer, uint64_t loc, size_t count) {
	...
	if (uninstall_data_i >= UNINSTALL_DATA_MAX) {
		fprintf(stderr,
				"error: Too many uninstall data entries! Please report this bug upstream.\n");
		return false;
	}
...
static int bios_install(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0) {
			...
			} else if ((device = fopen(argv[i], "r+b")) == NULL) {
	...
	if (gpt && gpt2mbr_allowed == true) {
		...
		for (int64_t i = 0; i < (int64_t) ENDSWAP(gpt_header.number_of_partition_entries); i++) {
			...
			if (ENDSWAP(gpt_entry.ending_lba) > UINT32_MAX) {
				if (!quiet) {
					fprintf(stderr,
							"Ending LBA of partition %" PRIi64
							" is greater than UINT32_MAX, will not convert GPT.\n",
							i + 1);
				}
				goto no_mbr_conv;
			...
			if (part_to_conv[part_to_conv_i].lba_end - part_to_conv[part_to_conv_i].lba_start + 1 >
				UINT32_MAX) {
				if (!quiet) {
					fprintf(stderr,
		...
		for (size_t i = 0; i < part_to_conv_i; i++) {
			...
			uint32_t sect_count =
				ENDSWAP((uint32_t) ((part_to_conv[i].lba_end - part_to_conv[i].lba_start) + 1));
...
no_mbr_conv:;
	...
	if (gpt) {
		...
		if (part_ndx != NULL) {
			...
			uint64_t entry_off =
				(uint64_t) partition_num * ENDSWAP(gpt_header.size_of_partition_entry);
			...
			if (!force && memcmp("Hah!IdontNeedEFI", &gpt_entry.partition_type_guid, 16) != 0) {
				fprintf(stderr, "error: Chosen partition for BIOS boot code is not of BIOS boot "
								"partition type.\n");
			...
			for (partition_num = 0; partition_num < ENDSWAP(gpt_header.number_of_partition_entries);
				 partition_num++) {
				uint64_t entry_off =
					(uint64_t) partition_num * ENDSWAP(gpt_header.size_of_partition_entry);
				if (add_u64_overflow(gpt_part_entry_base, entry_off, &entry_off)) {
				...
				if (memcmp("Hah!IdontNeedEFI", &gpt_entry.partition_type_guid, 16) == 0) {
					if (!quiet) {
						fprintf(stderr,
								"Autodetected partition %" PRIu32 " as BIOS boot partition.\n",
								partition_num + 1);
					}
			...
			fprintf(stderr,
					"error: Installing to a GPT device, but no BIOS boot partition specified or\n");
