...
int vfs_import_boot_file(const char *name, uint64_t size) {
    if (!name || !name[0] || size == 0) return -1;
    if (vfs_lookup_child(root_node, name)) return 0;
    struct vfs_node *n = new_node(name, VFS_NODE_BOOT_FILE, S_IFREG|0444);
    if (!n) return -1;
    n->data = 0;
    n->size = size;
    n->capacity = size;
...
size_t vfs_read_file(const char *path,char *out,size_t cap){
    ...
    const char *s=0;
    if(eq(path,"/proc/version")) s="YabroOS-32 0.0.1-alpha (procfs+musl-abi)\n";
