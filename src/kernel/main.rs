...
const FONT: [u8; 768] = [
    ...
    0x3C,0x60,0x7C,0x66,0x66,0x66,0x3C,0x00, 0x7E,0x06,0x0C,0x18,0x18,0x18,0x18,0x00,
    ...
    0x30,0x30,0x78,0x30,0x30,0x34,0x18,0x00, 0x00,0x00,0x66,0x66,0x66,0x66,0x3E,0x00,
...
pub extern "C" fn console_fb_putc(ch: u8) {
    unsafe {
        ...
        let max_cols = (fb.width / 12) as usize;
        let max_rows = (fb.height / 12) as usize;
...
unsafe fn draw_char(fb: &LimineFramebuffer, ch: u8, col: usize, row: usize) {
    ...
    let start_x = col * 12;
    let start_y = row * 12;
    let font_idx = if ch >= 32 && ch <= 127 { ((ch - 32) as usize) * 8 } else { 0 };
    for dy in 0..12 {
        let y = (dy * 2) / 3;
        let font_row = FONT[font_idx + y];
        for dx in 0..12 {
            let x = (dx * 2) / 3;
            let color = if (font_row & (1 << (7 - x))) != 0 {
                0x00_FF_FF_FF
            } else {
                0x00_00_00_00
            };
...
unsafe fn cluster_to_ptr(cluster: u32) -> *const u8 {
    ...
    FAT32_BASE.offset(offset as isize)
}
...
unsafe fn next_cluster(cluster: u32) -> u32 {
    ...
    let ptr = FAT32_BASE.offset((fat_sector_offset + fat_offset) as isize) as *const u32;
...
unsafe fn find_file_in_root(path: *const u8) -> Option<DirectoryEntry> {
    ...
    find_file_in_directory(FAT32_ROOT_CLUSTER,&raw[start..n])
}

unsafe fn get_file_size_by_name(name: &[u8]) -> usize {
    let root_cluster = FAT32_ROOT_CLUSTER;
    if let Some(entry) = find_file_in_directory(root_cluster, name) {
        return entry.file_size as usize;
    }
    0
}
...
unsafe fn import_fat_root_into_vfs() {
    ...
    loop {
        ...
        for entry in entries {
            ...
            if (e.attributes&0x10)!=0 { lfn[0]=0; continue; }

            let mut short=[0u8;260];
            let short_len=fat_short_name(e,&mut short);
            let name=if lfn[0]!=0 { &lfn[..] } else { &short[..short_len] };
            if name.len()>0 && name[0]!=0 {
                let _=vfs_import_boot_file(name.as_ptr(),e.file_size as u64);
            }
            lfn[0]=0;
        }
        let next=next_cluster(cur); if next>=0x0FFFFFF8 { break; } cur=next;
    }
}
...
pub unsafe extern "C" fn kernel_execve_file_args(path: *const u8,
    ...
    static mut EXEC_IMAGE: [u8; 262144] = [0u8; 262144];

    let image_ptr = core::ptr::addr_of_mut!(EXEC_IMAGE) as *mut u8;
    let image = core::slice::from_raw_parts_mut(image_ptr, 262144);
...
pub extern "C" fn kernel_main() -> ! {
    unsafe {
        ...
        let fb_virt = if fb_phys < 0x0000_8000_0000_0000 {
            hhdm.saturating_add(fb_phys)
        } else {
            fb_phys
        };
            ...
            if key == 13 {
                ...
                if buf_idx > 0 {
                    ...
                    if cmd_slice == b"help" {
                        ...
                        let fetch_text = b"\nYabroOS-32 Kernel v0.0.1-alpha (with ELF loader)\nRust + C Hybrid Kernel\nArchitecture: x86_64 / Limine\n\n";
