...
static inline void debug_hex(const char *s, uint64_t v) { (void)s; (void)v; }
...
static uint64_t *ensure_user_child(uint64_t *parent, uint64_t idx)
{
    ...
    if (!(e & PTE_U))
        parent[idx] = (e & PTE_ADDR_MASK) | PTE_P | PTE_W | PTE_U;
    return child;
}
...
int vmm_user_map(uint64_t virt, uint64_t phys, uint64_t flags)
{
    ...
    if ((*leaf & PTE_P) && ((*leaf & PTE_ADDR_MASK) == (phys & PTE_ADDR_MASK))) {
        *leaf = new_pte;
        return 0;
    }
    *leaf = new_pte;
