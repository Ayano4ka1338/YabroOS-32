...
static int cmd_wc(int ac,char**av){if(ac!=3){put("usage: wc FILE\n");return 2;}long fd=sc3(SYS_open,(long)av[2],0,0);if(fd<0){err("wc",av[2]);return 1;}char b[512];long lines=0,words=0,bytes=0,inword=0;for(;;){long n=sc3(SYS_read,fd,(long)b,sizeof(b));if(n<=0)break;bytes+=n;for(long i=0;i<n;i++){if(b[i]=='\n')lines++;if(b[i]==' '||b[i]=='\t'||b[i]=='\n'||b[i]=='\r')inword=0;else if(!inword){words++;inword=1;}}}sc1(SYS_close,fd);num(lines);put(" ");num(words);put(" ");num(bytes);put(" ");put(av[2]);put("\n");return 0;}
...
static int cmd_grep(int ac,char**av){if(ac<4){put("usage: grep PATTERN FILE\n");return 2;}long fd=sc3(SYS_open,(long)av[3],0,0);if(fd<0){err("grep",av[3]);return 2;}char b[4096];long total=0;for(;;){long r=sc3(SYS_read,fd,(long)b+total,sizeof(b)-1-total);if(r<=0)break;total+=r;if(total==(long)sizeof(b)-1)break;}sc1(SYS_close,fd);long plen=slen(av[2]),st=0;int found=0;for(long i=0;i<=total;i++){if(i==total||b[i]=='\n'){long len=i-st,match=0;for(long j=0;j+plen<=len;j++){long k=0;while(k<plen&&b[st+j+k]==av[2][k])k++;if(k==plen){match=1;break;}}if(match){putn(b+st,len);put("\n");found=1;}st=i+1;}}return found?0:1;}
...
static int dispatch(int ac,char**av){
    ...
    if(!applet && ac>=2 && (eq(av[1],"--help")||eq(av[1],"--list"))){
        put("YabrusBox YabroOS-32 0.0.1-alpha\n");
